...
#   ok=valid | badhead=head not hex | badlines=sum_lines mismatch
#            | badcount=count not numeric | badpair=weighted/final pairing broken
putcache() {
    ( cd "$T/$1/work"
      local v="$2" h1 body blob tree cc
...
newrepo C; putcache C badhead;  check C 1 EMPTY 0  # head not hex → full + drop
newrepo D; putcache D badlines; check D 1 EMPTY 0  # sum_lines mismatch → full + drop
...
newrepo G; putcache G badpair;  check G 1 EMPTY 0  # weighted/final pairing broken → header bad
